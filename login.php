...
if(isset($_POST['email']) && isset($_POST['pass'])){
	...
	$pass = $_POST['pass'];
	$emailchk=mysql_query("select email,phone from user where (email='$log' || phone='$log') AND fix='TDUID-'");
	if((mysql_num_rows($emailchk))>0){
			$sql="select * from user where (email='$log' || phone='$log') && Pass='$pass' AND fix='TDUID-'";
			...
			if($re=mysql_fetch_assoc($result)){
				...
				if($status==1){
					...
					if(isset($SID) && $SID!='' && $_SESSION['email']!='')
					{
