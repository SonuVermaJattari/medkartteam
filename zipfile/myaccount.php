...
if($_SESSION['email']==''){
	...
	if(isset($_GET['Delete'])){
		...
		$del=mysql_query("SELECT address.id,user.email FROM `address` INNER JOIN user ON address.u_id = user.id where address.id='$id' && user.email='$email'");
	...
	if(isset($_GET['default'])){
		 ...
		 $resAdd=mysql_query("SELECT address.id,user.email,user.id as user_id,user.address_id FROM `address` INNER JOIN user ON address.u_id = user.id where address.id='$id' && user.email='$email'");
		if(mysql_num_rows($resAdd)){
			$rsAdd=mysql_fetch_assoc($resAdd);
			//$address_id=$rsAdd['address_id'];
			//$id=$rsAdd['id'];
			$DB->executupdate("UPDATE `user` SET `address_id`='$id' WHERE id='".$rsAdd['user_id']."'");
	...
	$fetch="select * from user where email='$email' AND fix='TDUID-'";
