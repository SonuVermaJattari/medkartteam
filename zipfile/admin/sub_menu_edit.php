...
if($link=='sub sub menu'){
$link='0';
}
if(empty($erromsg)){

$sqlqry="update sub_menu set menu='$menu',sub_menu='$sub_menu',link='$link',Products_name='$Products_name',text='$editor1'";
...
$QRY="select * from sub_menu where id='$eid'";
        ...
        <div class="wrapper row-offcanvas row-offcanvas-left">
            ...
            <aside class="right-side">                
                ...
                <section class="content-header">
                    <h1>
                   Sub Menu Edit
                        <small><?php echo $array[$web['menu']]; ?></small>
                    </h1>
                    <ol class="breadcrumb">
					 ...
					 <!-- general form elements -->
                            <div class="box box-primary">
                                <div class="box-header">
									...
									<button type="button" name="View" style="float: right;" onClick=" window.location ='sub_menu_view.php?q=<?php echo $web['menu']; ?>';" class="btn btn-primary">View Sub Menu</button>
							<button type="button" name="View" style="float: right;" onClick=" window.location ='sub_menu.php';" class="btn btn-primary">Add Sub Menu</button>
                                </div><!-- /.box-header -->
                                <?php if($erromsg!=''){ ?>
                               <!-- Alert-->
                                ...
                                <form role="form" method="post" name="form" enctype="multipart/form-data">
                                    <div class="box-body" >
									 <div class="form-group">
											...
											<!--	<option ><?php echo $web['menu']; ?></option>-->
												<?php $sqlu = "select * from menu where status='1' AND  link='0' order by sort ";
												$resultu = mysql_query($sqlu);
												while($drop = mysql_fetch_array($resultu)){ ?>
												<option value="<?php echo $drop['id'];?>" <?php echo $web['menu']==$drop['id']?'selected':"" ?> ><?php echo $drop['menu'];?></option>
												<?php }?>
									...
									<div class="form-group">
                                            ...
                                            <input type="text" name="sub_menu" class="form-control" placeholder="Eg: " value="<?php echo $web['sub_menu']; ?>"required/>
                                        </div> 
										 ...
										 <div class="btn-group form-group">
											...
											<button type="button" class="btn btn-primary  foo" value="product" onClick="display(this)" >Product</button>
											  <button type="button" class="btn btn-primary  foo sub_ban" value="sub sub menu" onClick="display(this)" >sub sub menu</button>
										</div>
										<div class="form-group">
											<label>Link</label>
											<input id="abc" type="text" name="link" class="form-control"  value="<?php echo $web['link']=='0'?"sub sub menu":$web['link'];?>" required />
										</div>
