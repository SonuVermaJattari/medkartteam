...
$services_name=(int)$_GET['q'];
	if(isset($_POST['Inactive']) && $bb!='')
	{
		foreach($bb as $act)
		{
			list($id,$name)=explode('@',$act);
			mysql_query("update slider set status='0' where id='$id'");
		}
	}
	...
	if(isset($_POST['Activate']) && $bb!='')
	{
		...
		{	
			list($id,$name)=explode('@',$act);
			mysql_query("update slider set status='1' where id='$id'");
		}
	}
	...
	$mqry="select * from slider ORDER BY sort";
    ...
    <body class="skin-blue">
		...
		<div id="tableheader">
            ...
            <span class="details">
				...
				<div class="btn-reset"><button type="button" name="View" style="float: right;" onClick=" window.location ='slider.php';" class="btn btn-primary">Add Slider</button></div>
        	</span>
        </div>
        <section>
        ...
        <table cellpadding="0" cellspacing="0" border="0" id="table" class="tinytable">
            <thead>
                <tr>
					...
					<th class="nosort" style="text-align:center"><h3>Images</h3></th>
			  ...
			  while($web=mysql_fetch_array($fetch)) { ?>
                <tr>
					...
					<td style="text-align:center" ><img src="../<?php echo $web['img'];?>"  style="width:150px; height:70px;"  /></td>
                    <td style="text-align:center"><?php if( $web['status']=='1'){echo "Active";} else {echo "Inactive";}?></td>
                    <td style="text-align:center"><a href="slider_edit.php?eid=<?php echo $web['id'];?>" title="Edit">Edit</a></td>
