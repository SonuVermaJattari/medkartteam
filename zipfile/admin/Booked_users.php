...
if(isset($_POST['Activate']) && $bb!='')
{
		...
		{
			list($id,$name)=explode('@',$act);
			mysql_query("update approve set status='1' where id='$id'");
		}
}
...
if(isset($_POST['Delete']) && $bb!='')
{
		...
		{
			list($id,$name)=explode('@',$act);
			mysql_query("delete from approve where id='$id'");
			unlink($name);
		}
}
		...
		$mqry="SELECT approve.id,approve.client_id,user.fname,user.email,user.phone,approve.start as start_date,approve.end as end_date,COUNT(approve.room_id) as room_type,user.room,user.total_room as no_room,user.total_price,user.adult,user.child FROM `approve` left JOIN user on user.id=approve.client_id where approve.approve='1' group by approve.client_id ORDER by approve.start";
		 //where mark='main' ";
...
<html>
    <head>
        <meta charset="UTF-8">
        <title>Approve | Data Tables</title>
    ...
    <body class="skin-blue">
       ...
       <?php $room_type=$DB->fectch_prices('SELECT id,type FROM `room`'); print_r($fectch_prices);?>
            ...
            <thead>
			  ...
			  while($web=mysql_fetch_array($fetch)) { ?>
                <tr>
                    ...
                    <td><a href="users_edit.php?client_id=<?php echo $web['client_id']; ?>" title="Profile"><?php echo $web['fname'];?></a></td>
                    ...
                    <?php 
					$no_days=$DB->date_diff($web['start_date'],$web['end_date']);					
                    $i=1;
                    ...
                    <tr class="tax_table">
                    <td class=""><span class="imp_table_text">Booking Date : </span><br><a href="date_change.php?client_id=<?php echo $web['client_id']; ?>" title="Edit"><?php echo $DB->date_format1($web['start_date']); ?>  TO  <?php echo $DB->date_format1($web['end_date']); ?></a><br />
                    <span class="imp_table_text">Adult : <?php echo $web['adult']; ?></span> <span class="imp_table_text">Children : <?php echo $web['child']; ?></span><br></td>
                    <td class=""><span class="imp_table_text"><?php echo $web['no_room'];?> Rooms, <?php echo $no_days; ?> Days</span></td>
                    </tr>
                    <tr class="tax_table">
                    <td class="">Total</td>
                  ...
                  <!--  <td><a href="users_edit.php?client_id=<?php echo $web['client_id']; ?>" title="Edit">Edit</a></td>-->
                    <!--<td align="center"><input type="checkbox" value="<?php echo $web['id'].'@../'.$web['image']; ?>" name="bb[]" id="bb[]"></td>-->
                </tr>
               <?php $count++; }?>
            </tbody>
        </table>
