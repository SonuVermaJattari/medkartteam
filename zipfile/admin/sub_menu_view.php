...
if(isset($_POST['Inactive']) && $bb!='')
{
		foreach($bb as $act)
		{
		mysql_query("update sub_menu set status='0' where id='$act'");
		}
}
...
if(isset($_POST['Activate']) && $bb!='')
{
		foreach($bb as $act)
		{
			mysql_query("update sub_menu set status='1' where id='$act'");
		}
}
...
if(isset($_POST['Delete']) && $bb!='')
{
		foreach($bb as $act)
		{
			mysql_query("delete from sub_menu where id='$act'");
		}
}
		
		$mqry="select * from sub_menu where menu='$services_name' order by sort ";
		 //where mark='main' ";
    ...
    <body class="skin-blue">
            ...
            <aside class="right-side">
                ...
                <section class="content-header">
                      ...
                      Sub Menu <small><?php 
							  	...
							  	$resu= mysql_query("select * from menu where id=$services_name");
								$mees = mysql_fetch_array($resu);
									
							
					  
					  echo $mees['menu'];?></small>
                    </h1>
		...
		<div id="tableheader">
            ...
            <span class="details">
        		...
        		<div class="btn-reset"><button type="button" name="View" style="float: right;" onClick=" window.location ='sub_menu.php';" class="btn btn-primary">Add Sub Menu</button></div>
        	</span>
        </div>
        <section>
        ...
        <table cellpadding="0" cellspacing="0" border="0" id="table" class="tinytable">
            <thead>
                <tr>
					...
					<th class="nosort" style="text-align:center"><h3>Sort</h3></th>
					<th class="nosort" style="text-align:center"><h3>Sub Menu</h3></th>
					<th class="nosort" style="text-align:center"><h3>Link</h3></th>
			  ...
			  while($web=mysql_fetch_array($fetch)) { ?>
                <tr>
					...
					<td style="text-align:center;"><?php echo $web['sub_menu'];?></td>
					<td style="text-align:center"><?php echo $web['link']=='0'?"sub sub menu":$web['link'];?></td>
                    <td style="text-align:center;"><?php if( $web['status']=='1'){echo "Active";} else {echo "Inactive";}?></td>
                    <td style="text-align:center;"><a href="sub_menu_edit.php?eid=<?php echo $web['id'];?>" title="Edit">Edit</a></td>
