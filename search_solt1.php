...
<?php
	...
	if($search_drop=='1' || $search_drop=='0'){
		$menu_fetch=mysql_query("SELECT p.id,p.menu,p.name,cn.name as c_name,pa.name as p_bname, pa1.name as p_b1name FROM `products` p left JOIN packing pa on p.p_b=pa.id left JOIN packing pa1 on p.p_b1=pa1.id left JOIN company_name cn on p.company_name=cn.id  where  p.name LIKE '%$q%' LIMIT 0,$lim ");
		while($web=mysql_fetch_assoc($menu_fetch)) {
			 if ($hint=="") {
			  $hint="<a class='search' href='" . $web['id'] ."' target='_blank'>" .$web['name']. "</a>";
			} else {
			  $hint=$hint . "<br /><a class='search' href='" .$web['id'] ."' target='_blank'>" .$web['name']. "</a>";
			}
		}
	}
	if($search_drop=='2' || $search_drop=='0'){
		...
		$menu_fetch=mysql_query("SELECT p.id,s.name as solt,COUNT(p.id) FROM `products` p left join solt s ON p.solt=s.id where s.name LIKE '%$q%' GROUP by s.name LIMIT 0,$lim ");
		while($web=mysql_fetch_assoc($menu_fetch)) {
			//$hint.='<h1 style="color:#fff;">Solt</h1>';
			 if ($hint=="") {
			  $hint="<a class='search' href='" . $web['id'] ."' target='_blank'>" .$web['solt'].'(solt1)'. $web['COUNT(p.id)']."</a>";
			} else {
			  $hint=$hint . "<br /><a class='search' href='" .$web['id'] ."' target='_blank'>" .$web['solt'].'(solt2)'.$web['COUNT(p.id)']. "</a>";
			}
		}
	}
...
<div class="row">
												<div class="col-md-8"><div class="description">
