...
if(isset($_POST['submit']))
{
	...
	if($no_days>0){
		...
		foreach($_SESSION['room_admin'] as $key=>$val){ 
			$price=$fectch_prices[$key]['price']*$val;
			$total_price=$total_price+$price;
		}
		$total_price=$total_price*$no_days;
		$query="update `user` set `date_start`='$date_start',`date_end`='$date_end' , total_price='$total_price' where id='$client_id'";
...
$mqry="SELECT * FROM `user` where id='$client_id' AND active='0' ";
